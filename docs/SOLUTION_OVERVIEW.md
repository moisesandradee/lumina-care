...
Lumina does not use raw LLM inference for risk assessment. AI interpretation is constrained by validated clinical instruments (PHQ-9, GAD-7, C-SSRS) and structured prompt templates that encode clinical logic. The result is AI that reasons _with_ clinical evidence, not independently of it.
